...
export default function AdminSettings() {
  ...
  const handleSavePw = async () => {
    ...
    if (pwForm.currentPassword) {
      ...
      if (hashedCurrent !== creds.password) { setPwErrors({ currentPassword: 'Password saat ini salah' }); return; }
    }
      ...
      <div className="max-w-2xl space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            <button onClick={handleExport} className="btn-primary w-full justify-center">
              <Download className="w-4 h-4" /> Export Data Berita (JSON)
            </button>
            <label className="btn-secondary w-full justify-center cursor-pointer">
              <Upload className="w-4 h-4" /> Import Data dari File JSON
        ...
        <div className="bg-white rounded-2xl border border-red-200 p-6">
          ...
          <button onClick={() => setShowReset(true)} className="btn-danger">
            <RefreshCw className="w-4 h-4" /> Reset Semua Data ke Default
          </button>
        </div>
      </div>
    </AdminLayout>
